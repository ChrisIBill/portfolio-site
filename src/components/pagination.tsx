...
export default function CustomPagination() {
  ...
  const renderItem = ({
    ...
    if (
      value === pageIndex + 1 ||
      (pageIndex === InternalLinks.length && value === 1)
    ) {
      return (
        <PaginationItem
          key={key}
    ...
    if (
      value === pageIndex - 1 ||
      (pageIndex === 1 && value === InternalLinks.length)
    ) {
      return (
        <PaginationItem
          key={key}
...
const PaginationItem: React.FC<PaginationItemProps> = (props: any) => {
  ...
  const [backgroundColor, setBackgrounColor] = React.useState(
    "rgba(253, 255, 204, 0)",
  );
  ...
  useEffect(() => {
    if (props.type === "active") {
      ...
      setOpacity(0.5 + indicatorPosition);
    } else if (props.type === "prev" && indicatorPosition < 0) {
      setOpacity(0.5 + Math.abs(indicatorPosition));
    } else setOpacity(0.5);
  ...
  useEffect(() => {
    setBackgrounColor(
      theme === "dark"
        ? `rgba(253, 255, 204, ${1 - opacity})`
        : `rgba(24, 24, 27, ${1 - opacity})`,
    );
  }, [theme, opacity]);
  ...
  return (
    <button
      ...
      style={{
        boxShadow:
          !isActive && Math.abs(opacity) > 0.5
            ? `0 0 10px 0 rgba(253, 255, 204, ${opacity})`
            : "",
      }}
      onClick={handleClick}
    >
