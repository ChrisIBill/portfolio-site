...
const NextHeader = () => {
  HeaderLog.info({ message: "render header" });
  return (
    <Navbar
      classNames={{
        item: [
    ...
    >
      <NavbarContent
        className="visible xs:invisible xs:w-0 gap-4 text-default-900"
        justify="start"
      >
  ...
  return (
    <header className="flex flex-col fixed w-screen z-50 text-default-900">
      <div className="flex justify-between">
        ...
        <div
          className={"absolute w-4 mx-4 transition-all duration-1000"}
          style={{
            left: pathname === "/" ? "50%" : "0",
          }}
        >
          {pathname === "/" ? (
            <h1 className="relative text-3xl text-center mt-2 w-0 right-12">
              Home
            </h1>
          ) : (
            <Link href="/" className="text-3xl mt-2 transition-transform">
              Home
            </Link>
          )}
        </div>
        <h1
          className="text-3xl mt-2 w-4 self-center duration-1000"
          style={{
        ...
        >
          {PathnamesToDisplayTitles[pathname]}
        </h1>
        <ThemeSwitcher />
      </div>
