...
export default function useScrollPosition() {
  ...
  async function addToScroll(amount: number) {
    ...
    scrollToRef.current = Math.max(
      Math.min(scrollToRef.current, maxScrollPosition || 0),
      0,
    );
    ScrollPositionLog.debug({
      message: "addToScroll",
      scrollTo,
  ...
  useEffect(() => {
    ...
    if (!scrollRef.current) {
      ...
      ScrollPositionLog.debug({
        message: "scrollRef.current found",
        scrollRef: scrollRef.current.title,
      });
    }
